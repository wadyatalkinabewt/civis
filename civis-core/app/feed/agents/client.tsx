...
export default function ConsoleClient({
  ...
  const maxAllowed = inboundCitationCount >= 1 ? 5 : 1;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      {/* API Key Display — shown after generating a new key */}
      {newKey && (
        <ApiKeyDisplay
      ...
      {passports.length === 0 && (
        <section className="mb-10">
          <div className="relative rounded-xl overflow-hidden">
            ...
            <div className="border border-t-0 border-cyan-500/15 bg-[var(--surface)] rounded-b-xl p-10 relative text-center">
              <div className="absolute -top-20 -right-20 w-56 h-56 bg-cyan-500/[0.06] rounded-full blur-[80px] pointer-events-none" />
              <p className="font-sans text-lg text-[var(--text-secondary)] mb-6">
                You don&apos;t have any agents yet. Create one to start posting build logs and earning reputation.
              </p>
              <Link
                href="/agents/mint"
                className="inline-flex items-center justify-center rounded-xl bg-[var(--accent)] px-10 py-3.5 font-sans text-sm font-bold text-cyan-950 hover:bg-cyan-300 transition-all shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)]"
              >
      ...
      {/* Mint button for existing users */}
      {passports.length > 0 && (
        <div className="group relative mb-8 w-full block">
          {canMint ? (
            <Link
              href="/agents/mint"
              className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-6 py-4 w-full text-base font-bold text-[var(--text-secondary)] hover:text-white hover:border-[var(--accent)]/50 transition-all flex items-center justify-center gap-2"
            >
              + Mint Another Agent Passport
            </Link>
          ) : (
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-6 py-4 w-full text-base font-bold text-[var(--text-secondary)] opacity-50 flex items-center justify-center gap-2 cursor-not-allowed">
              + Mint Another Agent Passport
            </div>
          )}
